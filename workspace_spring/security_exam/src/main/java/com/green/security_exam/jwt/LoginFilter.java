...
public class LoginFilter extends UsernamePasswordAuthenticationFilter {
  ...
  public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException {
    ...
    //AuthenticationManager 가 담당하기 때문에 전달받은 아이디와 비밀번호를 AuthenticationManager 에 전달해줘야 한다.
    //이때 아이디와 비밀번호를 그냥 전달하는 것이 아니라 UsernamePasswordAuthenticationToken 객체(통)에 실어 보낸다.
    UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(dto.getMemEmail(), dto.getMemPw(), null);

    //아이디와 비번을 담고 있는 authToken 객체 를 authenticationManager 에 전달, authenticationManager 는 로그인을 검증하는 기능을 함
    //로그인을 검증하는 방법 -> UserDetailsService 의 loadUserByUsername 메서드를 호출하여 검증
    ...
    //로그인 유저의 정보가 담긴 authentication 객체를 리턴하면 authentication 객체가 session 에 저장됨
    //세션에 저장하는 이유는 security 의 권한 처리를 위해서는 세션에 로그인 정보가 있어야 되기 때문.
    return authentication; // 받으면 Security : AuthenticationManager 에게 전달 > 이후 검증 시작!
  }

  // 인증 성공 시 실행되는 메서드 authResult : 로그인 인증 성공한 사람의정보가 다 들어가있음
  @Override
  protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
    ...
    String role = auth.getAuthority(); //로그인한 사람의 권한!!!

    String clientType =  request.getHeader("clientType"); // app > app , web > null
    clientType =  clientType==null? "web":clientType;

    //토큰 생성
    String accessToken = jwtUtil.createJwt(username, role, (1000 * 60 * 30),clientType); //30분 (1000==1초 /60==1분)

    //생성한 토큰을 응답 헤더에 담아 클라이언트에 전달
    response.setHeader("Access-Control-Expose-Headers", "Authorization"); // Authorization 접근권한 해제시켜 보여줌
    response.setHeader("Authorization", "Bearer " + accessToken); // "Authorization" : 접근권한 "Bearer ": 띄어쓰기를 꼭 한다.
